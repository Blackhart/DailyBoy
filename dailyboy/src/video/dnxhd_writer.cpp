/*!
 * \file dnxhd_writer.cpp
 * \brief FFmpeg DNxHD/DNxHR MOV encode: RGB ImageBuf → YUV, one frame at a time.
 */
...
const char* profile_name(
    JobOutputVideoDnxhd::JobOutputVideoDnxhdProfileValue v) {
...
AVPixelFormat dnxhd_av_pix_fmt(
    JobOutputVideoDnxhd::JobOutputVideoDnxhdPixFmtValue v) {
...
void encode_pad(const JobOutputVideoDnxhd& options, int width, int height,
                int* encode_width, int* encode_height) {
  using Pix = JobOutputVideoDnxhd::JobOutputVideoDnxhdPixFmtValue;
  *encode_width = width;
  *encode_height = height;
  if (options.pix_fmt() != Pix::Yuv444p10 && (width % 2) != 0) {
...
Status apply_pix_fmt(AVCodecContext* codec,
  ...
  codec->pix_fmt = dnxhd_av_pix_fmt(options.pix_fmt());
  using Pix = JobOutputVideoDnxhd::JobOutputVideoDnxhdPixFmtValue;
  const char* name = "yuv422p";
  if (options.pix_fmt() == Pix::Yuv422p10) {
    name = "yuv422p10";
  } else if (options.pix_fmt() == Pix::Yuv444p10) {
    name = "yuv444p10";
  }
...
Status apply_bitrate_kbps(AVCodecContext* codec,
  ...
  if (options.bitrate_kbps() > 0) {
    ...
    log_debug("encode: bitrate_kbps unused (dnxhr)");
  }
  return Status::Ok();
}

Status apply_interlaced(AVCodecContext* codec,
                        const JobOutputVideoDnxhd& options) {
  log_debug(std::string("encode: interlaced ") +
...
Status apply_profile(AVCodecContext* codec,
  ...
  if (av_opt_set(codec->priv_data, "profile", name, 0) < 0) {
    return ffmpeg_error("failed to set dnxhd profile.");
  }
  return Status::Ok();
}

Status apply_nitris_compat(AVCodecContext* codec,
                           const JobOutputVideoDnxhd& options) {
  log_debug(std::string("encode: nitris_compat ") +
            (options.nitris_compat() ? "true" : "false"));
  if (av_opt_set_int(codec->priv_data, "nitris_compat",
                     options.nitris_compat() ? 1 : 0, 0) < 0) {
    return ffmpeg_error("failed to set dnxhd nitris_compat.");
  }
  return Status::Ok();
}

Status apply_dnxhd_codec_options(AVCodecContext* codec,
                                 const JobOutputVideoDnxhd& options) {
  DAILYBOY_RETURN_IF_ERROR(apply_pix_fmt(codec, options));
  DAILYBOY_RETURN_IF_ERROR(apply_bitrate_kbps(codec, options));
  DAILYBOY_RETURN_IF_ERROR(apply_interlaced(codec, options));
  DAILYBOY_RETURN_IF_ERROR(apply_profile(codec, options));
  DAILYBOY_RETURN_IF_ERROR(apply_nitris_compat(codec, options));
  return Status::Ok();
}

}  // namespace

struct DnxhdWriter::AvState {
...
void DnxhdWriter::reset_av(AvState& av) {
  if (av.yuv != nullptr) {
...
DnxhdWriter::DnxhdWriter() = default;

DnxhdWriter::~DnxhdWriter() { close(); }
...
Status DnxhdWriter::open(const std::filesystem::path& path, int width,
  ...
  pts_ = 0;
  interlaced_ = options->interlaced();
  ...
  if (codec == nullptr) {
    return fail(ffmpeg_error("dnxhd encoder not found."));
  }
  ...
  av_->codec->codec_id = AV_CODEC_ID_DNXHD;
  ...
  DAILYBOY_RETURN_IF_ERROR(
      fail(apply_dnxhd_codec_options(av_->codec, *options)));
  ...
  if (interlaced_) {
    av_->yuv->flags |= AV_FRAME_FLAG_INTERLACED | AV_FRAME_FLAG_TOP_FIELD_FIRST;
  }
...
Status DnxhdWriter::write(const Frame& frame) {
